...
cap.set(cv2.CAP_PROP_FRAME_WIDTH, H_RESOLUTION)
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, V_RESOLUTION)
...
while cap.isOpened():
    ...
    img, detection = detector.plot_boxes(results,img,height=img.shape[0],width=img.shape[1],confidence=0.5)


    tracks = object_tracker.update_tracks(detection, frame=img)
    ...
    for track in tracks:
        ...
        cv2.rectangle(img,(x_center,y_center),(x_center,y_center),(0,255,255),2)
